...
RecommenderManhattan::RecommenderManhattan() {}
...
double computeDistance(const unordered_set<Rating, Rating::Hash>& user1,
    ...
    for (const Rating& r1 : user1) {
        auto it = user2.find(Rating{r1.movie, 0});  // Busca pelo ID do filme
        if (it != user2.end()) {
            sum += fabs(r1.score - it->score);
            common++;
        }
    }

    return (common > 0) ? sum : INFINITY;
}
...
vector<pair<int, double>> findSimilarUsers(int targetUserId,
    ...
    // Ordena por distância crescente (menor = mais similar)
...
vector<pair<int, float>> getRecommendations(int targetUserId,
                                           ...
                                           const UsersAndMoviesData& data) {
    const auto& targetRatings = data.at(targetUserId);
    unordered_map<int, pair<double, double>> recommendations;  // filme: (soma, peso)

    for (const auto& [similarUserId, distance] : similarUsers) {
        const auto& similarUserRatings = data.at(similarUserId);
        double weight = 1.0 / (1.0 + distance);  // menor distância => maior peso
    ...
    // Converte para pontuações finais
...
void RecommenderManhattan::generateRecommendations(const UsersAndMoviesData& usersAndMovies, const MoviesData& movies) {
    ...
    for(int currentUser : usersToRecommend) {
        ...
        for (const auto& [movieId, score] : recommendations) {
            ...
            if(it != movies.end()) {
                std::cout << it->second << " (Score: " << score << ")\n";
            }
        }
